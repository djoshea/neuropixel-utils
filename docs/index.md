...
- [Load](/imec_dataset/#constructing-a-neuropixelimecdataset) and [visualize](/imec_dataset/#plotting-specific-time-windows) raw neuropixel data from `imec.ap.bin` and `imec.lf.bin` files in Matlab
- [Write custom pre-processing functions](/imec_dataset/#building-a-preprocessing-pipeline) to apply to raw data either by writing a copy of the raw file or modifying it in place, optionally removing specific problematic time windows in the file
- [Concatenate multiple Imec data files](/imec_dataset/#concatenating-multiple-files-together) together while matching the amplifier gains
- [Run Kilosort/Kilosort2](/kilosort/#running-kilosort), and [load the results](/kilosort/#loading-kilosort-results) back into Matlab after manual inspection in Phy
- [Plot drift maps](/analysis/#plotting-drift-maps) using code adapted from the [spikes repository](https://github.com/cortex-lab/spikes)
- [Extract waveforms](/waveforms/#extracting-waveforms-via-kilosortdataset) for each cluster from the raw data, optionally cleaning the snippets by subtracting templates for other clusters spiking during the same time window
- [Visualize cluster electrical spiking images](/analysis/#plotting-electrical-images) in space and [cluster locations on the probe](/analysis/#plotting-cluster-centers-of-mass)
- [Determine trial boundaries](/kilosort/#segmenting-a-kilosort-dataset-into-trials) in the file, and efficiently (segment Kilosort results into individual trials)[/kilosort/#kilosorttrialsegmenteddataset]
