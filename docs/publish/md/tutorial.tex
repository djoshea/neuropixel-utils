...
\matlabheading{Startup}

\begin{par}
\begin{flushleft}
Edit this to point at the path to where you installed the \texttt{neuropixel-utils} distribution or repo.
\end{flushleft}
\end{par}
...
\begin{par}
\begin{flushleft}
Set the default channel map file you want to use with your data. Some standard channel maps are included in the \texttt{map\_files} directory in the \texttt{neuropixel-utils} distribution.
\end{flushleft}
\end{par}
...
\matlabheading{Load Raw Imec dataset}
...
\setlength{\itemsep}{-1ex}
   \item{\begin{flushleft} Path to raw \texttt{.imec.ap.bin} file or \texttt{imec.lf.bin} file. \end{flushleft}}
   \item{\begin{flushleft} Path to the containing directory, if there is only one set of \texttt{.bin} files within. \end{flushleft}}
\end{itemize}
...
\matlabheadingtwo{Specifying the channel map manually}

\begin{par}
\begin{flushleft}
By default, the channel map will be loaded from whatever the environment variable NEUROPIXEL\_MAP\_FILE is set to, which we set above. If you want to manaully specify a channel map, pass it in to the ImecDataset constructor using the \texttt{channelMap} option:
\end{flushleft}
\end{par}
...
\matlabheadingtwo{Explore the loaded metadata}

\begin{par}
\begin{flushleft}
Here we see that this option 3A probe has 384 total channels (saved in the \texttt{.imec.ap.bin} file), and that 374 of these channels are connected, namely those listed in \texttt{connectedChannels}.
\end{flushleft}
\end{par}
...
\matlabheading{Accessing the raw AP data}

\begin{par}
\begin{flushleft}
You can access the raw AP data in several ways. (See the full list of methods with \texttt{methods(imec)} or \texttt{doc npxutils.ImecDataset}). The most straightforward is to memory-map the full binary data:
\end{flushleft}
\end{par}

\begin{par}
\begin{flushleft}
\textit{This bit is broken because it references code that doesn't exist in this library. Probably left over from an older version of the library and never updated? Or maybe it has an external dependency?}
\end{flushleft}
\end{par}
